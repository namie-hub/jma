...
| `jma_early.js` | 警戒レベル1 早期注意情報, generated by `scripts/ingest_early_warning.py` on the atlas-ingest schedule. Optional: the Disaster Atlas degrades to levels 2–5 without it. |
...
## 警戒レベル — the five-step scale, and what issues each rung

The levels are Japan's shared evacuation-action scale, not JMA product names.
**Levels 3, 4 and 5 are evacuation orders issued by your municipality**; JMA
publishes the meteorological information that corresponds to each rung. The
28 May 2026 reform put the level number into JMA's own product names so the
correspondence no longer needs a lookup table.

| Level | Action (municipality) | Information (JMA) |
| --- | --- | --- |
| 5 | 緊急安全確保 | レベル5大雨特別警報 / 氾濫特別警報 |
| 4 | 避難指示 | レベル4大雨危険警報 / 土砂災害危険警報 / 氾濫危険警報 |
| 3 | 高齢者等避難 | レベル3大雨警報 / 土砂災害警報 / 氾濫警報 |
| 2 | Check hazard map and route | 大雨注意報 / 土砂災害注意報 / 氾濫注意情報 |
| 1 | Stay alert for updates | **早期注意情報（警報級の可能性）** |

**Level 1 is different in kind from the rest, and the Atlas treats it that
way.** Levels 2–5 declare what *is* in force. Level 1 is a forecast that JMA
*may* issue a warning inside a stated window, graded 高 or 中, reaching up to
two days ahead. It is drawn dashed and italic with a ◇ marker, counted
separately from the "active now" total, and never allowed to claim a chip in
a region that already has something in force — where it exists alongside a
warning, a small ◇ marks it and the panel carries the detail.

Level 1 does not appear in the r8 warning feed at all. It is a separate
product family (`VPFD61 早期注意情報（明後日まで）`) carried by the developer XML
*regular* feed, and the grading lives inside ~62 individual telegrams at
~23 kB each — so `scripts/ingest_early_warning.py` collects it on a schedule
and commits `jma_early.js`, rather than a browser pulling 1.4 MB to colour a
chip. If that file is absent the page says "level 1 unavailable" and levels
2–5 carry on unaffected; absence is reported as a gap, never as an all-clear.

Before this, a region under 警報級の可能性「高」 and a region with a clear week
ahead rendered identically. That conflation was the same species of error as
the frozen feed: two genuinely different states collapsing into one
reassuring display.
